...
public class ProductApiService {
    ...
    @RequestMapping("/test123product/{productId}")
    @HystrixCommand(fallbackMethod = "defaultProductComposite")
    public Map<String,Object> get123Product(
        ...
        LOG.info("GetProductComposite from URL: {}", url);

        ResponseEntity<String> result = restTemplate.getForEntity(url, String.class);
        LOG.info("GetProductComposite http-status: {}", result.getStatusCode());
        LOG.info("GetProductComposite body: {}", result.getBody());

        //return result;
        
		Map<String,Object> model = new HashMap<String,Object>();
		model.put("id", UUID.randomUUID().toString());
		model.put("content", "Hello 123 Pogikanga");
		return model;

    }

    @RequestMapping("/test456product/{productId}")
    @HystrixCommand(fallbackMethod = "defaultProductComposite")
    public Map<String,Object> getTest456Product(
        @PathVariable int productId,
        @RequestHeader(value="Authorization") String authorizationHeader,
        Principal currentUser) {
		...
		Map<String,Object> model = new HashMap<String,Object>();
		model.put("id", UUID.randomUUID().toString());
		model.put("content", "Hello 456 Pogikanga");
		return model;
